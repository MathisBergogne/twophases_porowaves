...
@views function Temperature_E(do_visu, do_p_itr,do_save, perm, visco_f, visco_r,dir_plots)
	# input
	...
	ρf		= 2600							# mvolumetric mass fluid (kg.m-3)
	Cps		= 1000
	Cpf		= 1000
	λs		= 2
	λf		= 2
	...
	for it = 1:nt
		...
		χT = avx(λT) ./ ρCpT
		# time step 
		dt_diff		= 0.5*dx^2 / maximum(χT) / 10
		dt_adv		= 0.1*dx/maximum(abs.(qD))
		dt = min(dt_adv,dt_meca)/10 # dt_diff #
		real_time   += dt
		...
		println("i for T loop : ",round(dt/dt_diff))
		...
		if (it % nvis) == 0 && do_visu 
			...
			if do_save savefig(@sprintf("%s/%04d",dir_plots,it)) end
		end
	end
end
...
dir_plots = "/media/bergogne/Disk_D/Documents/6-Julia/twophase_porowaves_output/wrong_T"

Temperature_E(do_visu, do_p_itr, do_save, perm, visco_f, visco_r, dir_plots)
