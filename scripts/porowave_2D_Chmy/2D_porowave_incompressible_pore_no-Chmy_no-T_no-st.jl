...
@views function porowave_1D_pore(do_visu)
	...
	ny   = 2000      # nombre de noeuds de modèles en y
	...
	for it = 1:nt 
	    ...
	    if (it % nvis) == 0 && do_visu || isnan(ϕ[2])
			  ploting_ϕ(xc,yc,ϕ,it,it*dt/year2sec) 
	    end
		if isnan(ϕ[2]); 
			println("Break : Nan")
			break
...
@views function ploting_ϕ(xc,yc,ϕ,it,real_time)
	display(heatmap(xc,yc,ϕ',title=@sprintf(" melt; time : %1.1e y",real_time);xlims=(xc[1],xc[end]),ylims=(yc[1],yc[end]),aspect_ratio=1,c=:turbo)) ## ,clim=(0,0.6)
	savefig(@sprintf("plot%d.png",it))
end
