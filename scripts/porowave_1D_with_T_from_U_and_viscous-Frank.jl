...
@views function extra_T(A)
	B = zeros(size(A,1)+1)
	B[2:end-1] = avx(A)
	B[1] = A[1] + (A[1] - A[2]) / 2 
	B[end] = A[end] + (A[end-1] - A[end-2]) / 2
	return B
end
...
@views function break_conditions(ϕ,T,T_init,real_time,nx,lim_bubb,dx,box_top)
	ϕ_heigh = bubble_heigh(ϕ, lim_bubb)
	if isnan(ϕ[2]) 
		println("break (φ = NaN at : ",real_time," y )")
		return true
	elseif minimum(ϕ) < 0
		neg_ϕ = findall(x->x<0, ϕ)
		println("break (négative porosity located at ", - neg_ϕ .* dx .- box_top , "; at : ",real_time," y )")
		return true 
	elseif ϕ_heigh <= (0.15 * nx)
		println("break (bublle rise the top at : ",real_time," y )")
		return true
...
@views function ploting(ϕ,ϕ_init,T,T_init,qD,ηf_r,η_r,xc,real_time,ϕ_heigh,dx,lx,box_top,it,year2sec,cte_η_f,cte_η_r,save_plots)
	...
	# set up plots
	p1 = plot([ϕ,ϕ_init].*100, xc, title="% of Melt", xlims=(0,max(0.65,maximum(ϕ))*110))
	...
	p4 = plot([T T_init], avx(xc), title="Temperature", xlims=(500,800))
	...
	elseif cte_η_r 
		...
		display(plot(p1,p2,p3,p4,p5,p6,plot_title=title_plot; layout=(2, 3)))
...
@views function Temperature_E(do_visu, do_p_itr, do_p_bb_h, do_p_vel,save_plots,cte_η_f,cte_η_r, perm, η0_r, θ_r, η0_f, θ_f)
	...
	for it = 1:nt
		...
		χT = avx(λT) ./ ρCpT
		#test eta Frank
		η_r		= η0_r .* exp.(- θ_r .* extra_T(T))	# matrix viscosity (Pa.s)
	    ηf_r	= η0_f .* exp.(- θ_f .* extra_T(T))	# fluid viscosity (Pa.s)	
		η_ϕbg	= η_r / ϕ_bg					# Pa.s	
		...
		ϕ .-= dt * (1 .- ϕ) .* Pe ./ η_ϕ 
		#Break ?
		do_break = break_conditions(ϕ,T,T_init,real_time/year2sec,nx,lim_bubb,dx,box_top)
		if do_break
			nt = it
		...
		if (it % nvis) == 0 && do_visu
			...
			ploting(ϕ,ϕ_init,T,T_init,qD,ηf_r,η_r,xc,real_time,ϕ_heigh,dx,lx,box_top,it,year2sec,cte_η_f,cte_η_r,save_plots)
		end
	end
	if do_p_vel 
...
if cte_η_f
	η0_f	= 1e4 		# constant fluid viscosity
